...
import { submitLead } from "../lib/cms-client";
...
export default function ContactForm({
  compact = false,
  source = "contact",
  success = "Received! We'll reach out within 24 hours with your custom growth plan.",
}) {
    ...
    return (
      <form className="form-wrap" onSubmit={onSubmit}>
        ...
        <button className="form-submit" disabled={pending}>
          {pending ? "Sending..." : "Claim Free Audit →"}
        </button>
  ...
  return (
    <form className="form-wrap" onSubmit={onSubmit}>
      ...
      <button className="form-submit" disabled={pending}>
        {pending ? "Sending..." : "Request Free Strategy Session →"}
      </button>
      <div className={`success-msg${ok ? " show" : ""}`}>{success}</div>
    </form>
  );
