...
function serializeBlog(item) {
  const date = item.date || (item.publishedAt ? new Date(item.publishedAt).toLocaleDateString("en-IN", { month: "short", day: "numeric", year: "numeric" }) : "");
  return {
    ...item,
    date,
...
function serializeUpdate(item) {
  return {
    ...item,
    date: item.date || (item.publishedAt ? new Date(item.publishedAt).toLocaleDateString("en-IN", { month: "long", year: "numeric" }) : ""),
  };
}

export async function getSiteContent() {
  const [content, services, testimonials, cases, faqs, contactFaqs, blogPosts, updates, team, tools, partners, awards, whyPoints, process] = await Promise.all([
    getContentMap(),
    listServices(),
    listTestimonials(),
    listCaseStudies(),
  ...
  const site = {
    ...
    location: getPath(content, "footer.location", fallbackPayload.site.location),
    tagline: getPath(content, "footer.description", fallbackPayload.site.tagline),
  };
  ...
  return {
    ...
    team,
    tools,
    partners: partners.map((item) => (typeof item === "string" ? item : item.name)).filter(Boolean),
    awards,
    whyPoints,
    process,
  };
